...
Dernière mise à jour de cette documentation : 31 janvier 2022.
...
Le package **serad** (**S**ynthese d'**E**crits avec les **R**egles en vigueur **A** la **D**ares) fournit quelques fonctions facilitant l'écriture de textes dynamiques et respectant les règles de formattage en vigueur à la Dares.
...
```
Les options de `comparaison` sont multiples, et ce qui a permis de créer facilement des fonctions dérivées très pratiques : `alahausse(x1,x2)`, `audessus(x1,x2)`, `davantage(x1,x2)`, `depasse(x1, x2, sing)`... La liste s'allongera sûrement au fil des remontées d'utilisateurs.

Par ailleurs, une comparaison nominale est codée directement avec `g_nom_simple(x1,x2)`.


**Comparaisons plus fines**

Pour nuancer l'ampleur de l'évolution, deux fonctions s'imposent tout d'abord : `g_nom(x1,x2)` -et son avatar `g_nom0(g)`- pour une évolution nominale, et `g_verbe(x1,x2)` -et son avatar `g_verbe0(g)`- pour une évolution verbale.
```{r }
...
Au plus raffiné (mais sans faire directement suivre de l'évolution), on peut avoir un verbe qui tient compte à la fois de la variation et de l'accélération. Cela est permis par `gETa_verbe(x1,x2,x3,sing)` - et son avatar `gETa_verbe0(g1,g2,sing)`.
```{r }
?gETa_verbe
#Astuce : la fonction sous-jacente est gETa_verbe00. 
#Il sera ultérieurement possible de la paramétrer.
```
...
#En glissements annuels, le nombre de décès de passagers avant 
#`r gETa_verbe(y1,y2,y3,1)` (`r format_g(g(y1,y2))`, soit 
#`r format_delta(y1-y2)` décès, après `r format_g(g(y2,y3))`, 
#soit `r format_delta(y2-y3)`).
```
En glissements annuels, le nombre de décès de passagers à l'avant *`r gETa_verbe(y1,y2,y3,1)`* (`r format_g(g(y1,y2))`, soit `r format_delta(y1-y2)` décès, après `r format_g(g(y2,y3))`, soit `r format_delta(y2-y3)`). Le nombre de décès de passagers à l'arrière *`r g_verbe(z1,z2,1)`* (soit `r format_delta(y1-y2)` décès).
