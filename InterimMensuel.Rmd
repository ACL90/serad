...
Fin `r MoisPROV` `r AnneePROV`, `r format_niv(x1)` personnes occupent un emploi intérimaire (données provisoires, corrigées des variations saisonnières), soit `r g_nom(x1,x2)` de `r format_g(g(x1,x2),signe = 0)` (soit `r format_delta(x1-x2)` intérimaires) par rapport au mois précédent (après `r format_g(g(x2,x3))` en `r MoisRECT` `r AnneeRECT`, soit `r format_delta(x2-x3)` ;
après `r format_g(g(x3,x4))` en `r MoisRECT_1` `r AnneeRECT_1`, soit `r format_delta(x3-x2)` ;
après `r format_g(g(x4,x5))` en `r MoisRECT_2` `r AnneeRECT_2`, soit `r format_delta(x4-x3)` ;
après `r format_g(g(x5,x6))` en `r MoisRECT_3` `r AnneeRECT_3`, soit `r format_delta(x5-x4)`). 
...
```
Fin `r MoisPROV` `r AnneePROV`, l’emploi intérimaire `r g_verbe(x1,x2,1)` `r format_g(g(x1,x2),signe=0)` sur un an (soit `r format_delta(x1-x2)` intérimaires).
...
for (i in 1:nrow(Secteurs2)){
   cat(paste0(g_verbe(SecteursG[i,5][[1]],SecteursG[i,2][[1]],1),
       " ",
       format_g(g(SecteursG[i,5][[1]],SecteursG[i,2][[1]]),0),
       " dans ",
...
Sur un an, l’emploi intérimaire `r g_verbe0(Regions2[1,3],1)` `r format_g(Regions2[1,3],0)` `r Regions2[1,1]` (soit `r format_delta(Regions2[1,2])` intérimaires) ; il `r g_verbe0(Regions2[2,3],1)` `r format_g(Regions2[2,3],0)` `r Regions2[2,1]` (soit `r format_delta(Regions2[2,2])` intérimaires) ; il `r g_verbe0(Regions2[3,3],1)` `r format_g(Regions2[3,3],0)` `r Regions2[3,1]` (soit `r format_delta(Regions2[3,2])` intérimaires).
En revanche, l’emploi intérimaire `r g_verbe0(Regions2[A-1,3],1)` `r format_g(Regions2[A-1,3],0)` `r Regions2[A-1,1]` (soit `r format_delta(Regions2[A-1,2])` intérimaires) ; il `r g_verbe0(Regions2[A-2,3],1)` `r format_g(Regions2[A-2,3],0)` `r Regions2[A-2,1]` (soit `r format_delta(Regions2[A-2,2])` intérimaires) ; il `r g_verbe0(Regions2[A-3,3],1)` `r format_g(Regions2[A-3,3],0)` `r Regions2[A-3,1]` (soit `r format_delta(Regions2[A-3,2])` intérimaires).
